...
if (!FENIX_SESSION_SECRET) {
  throw new Error('SESSION_SECRET nÃ£o configurado para este ambiente.');
}
    ...
    notices: [
      {
        ...
        message: 'COMEÃƒâ€¡AR A LIVE 5 MINUTOS ANTES DO SEU HORARIO PROGRAMADO NO FENIX LURK',
        active: true,
...
function fenixSetExtraTargetFinal(data, number, payload, updatedBy) {
  ...
  if (!FENIX_EXTRA_TAB_NUMBERS_FINAL.includes(tabNumber)) {
    throw new Error("NÃºmero de aba extra invÃ¡lido.");
  }
...
app.post('/api/fenix/app/complete-cycle', fenixCycleRateLimitFinal, (req, res) => {
  ...
  // PontuaÃ§Ã£o vÃ¡lida somente no acumulado semanal.
...
function fenixNormalizeExtraTabsHeartbeatFinal(value) {
  ...
  return FENIX_EXTRA_TAB_NUMBERS_FINAL.map((number) => {
    return byNumber.get(number) || {
      ...
      currentUrl: "",
      status: "closed",
      detail: "Sem informaÃ§Ã£o do app.",
      pageLoaded: false,
      liveFound: false,
      playerFound: false,
      playing: false,
...
function fenixParseHours(value) {
  ...
  const regex = /(\d{1,2})\s*:?\s*00\s*(?:as|Ã s|a|-|atÃ©)\s*(\d{1,2})\s*:?\s*00/gi;
  let match;
...
function fenixParseApplicantsFromPaste(pastedText) {
  ...
  const idxBaixou = fenixFindColumn(headers, ['VocÃª jÃ¡ baixou', 'baixou']);
  const idxObs = fenixFindColumn(headers, ['ObservaÃ§Ã£o final', 'observacao', 'observaÃ§Ã£o']);

  const dayIndexes = {
    domingo: fenixFindColumn(headers, ['[DOMINGO]', 'DOMINGO']),
    segunda: fenixFindColumn(headers, ['[SEGUNDA]', 'SEGUNDA']),
    terca: fenixFindColumn(headers, ['[TERÃ‡A]', '[TERCA]', 'TERÃ‡A', 'TERCA']),
    ...
    sabado: fenixFindColumn(headers, ['[SÃBADO]', '[SABADO]', 'SÃBADO', 'SABADO'])
  };
...
<body>
  <header>
    ...
    <div class="muted">Importe os formulÃ¡rios, veja os inscritos e gere a grade respeitando os horÃ¡rios marcados.</div>
  </header>

  <main>
    <section>
      <h2>1. Importar formulÃ¡rio</h2>
      <p class="muted">Copie da planilha do Google Forms incluindo o cabeÃ§alho e cole aqui.</p>
    ...
    <section>
      ...
      <label>HorÃ¡rios vagos por dia para preencher manual no grupo:</label>
      ...
      <button id="btnLoadDraw" class="secondary" type="button">Ver Ãºltimo sorteio</button>
      <div id="drawMsg" class="muted"></div>
    </section>

    <section>
      <h2>4. Grade sorteada</h2>
      <div class="muted">Essa grade Ã© rascunho. Ainda nÃ£o altera a grade ativa do app.</div>
      <br>
      <div id="drawBox"></div>
    </section>
  </main>
...
<script>
  ...
  function copyVacancy(day, hour) {
    const msg =
      "ðŸ”¥ VAGA ABERTA NA GRADE FENIX ðŸ”¥\n\n" +
      "HorÃ¡rio: " + day + " Ã s " + hour + "\n" +
      "Vagas disponÃ­veis na grade.\n\n" +
      "Quem estiver em live nesse horÃ¡rio, marca âœ… aqui no grupo.";
  ...
  function renderDraw(draw) {
    ...
    let html =
      '<div class="card" style="margin-bottom:14px">' +
        ...
        '<div><b>Vagas automÃ¡ticas disponÃ­veis:</b> ' + Number(summary.totalAvailableScreens || 0) + '</div>' +
        '<div><b>Vagas necessÃ¡rias:</b> ' + Number(summary.requiredScreens || 0) + '</div>' +
        ...
        '<div class="card" style="margin-bottom:14px;border-color:#713333">' +
          ...
          below.map((item) => {
            return '<tr>' +
              ...
              '<td>' + Number(item.available || 0) + ' horÃ¡rios</td>' +
    ...
    html += "<table><thead><tr><th>Dia</th><th>Hora</th><th>Tela 1</th><th>Tela 2</th><th>Tela 3</th><th>AÃ§Ã£o</th></tr></thead><tbody>";
...
app.post('/admin/grade-sorteio/importar', (req, res, next) => {
  ...
  try {
    ...
    if (!parsed.length) {
      return res.type('html').send(`
        <body style="background:#090909;color:white;font-family:Arial;padding:30px">
          ...
          <p>Copie a planilha incluindo a linha do cabeÃ§alho.</p>
    ...
    return res.type('html').send(`
      <body style="background:#090909;color:white;font-family:Arial;padding:30px">
        <h1 style="color:#f3c451">Importado com sucesso âœ…</h1>
...
app.post('/api/fenix/admin/form-applicants/import', requireFenixAdmin, (req, res) => {
  ...
  if (!parsed.length) {
    return res.status(400).json({
      ok: false,
      message: 'Nenhum inscrito encontrado. Cole a planilha com o cabeÃ§alho.'
    });
  }
...
<header>
  ...
  <div class="muted">PÃ¡gina sem JavaScript. Aqui os botÃµes funcionam direto pelo backend.</div>
</header>
...
<main>
  ...
  <section>
    ...
    <form method="POST" action="/admin/grade-sorteio-simples/importar">
      ...
      <textarea name="text" placeholder="Cole aqui cabeÃ§alho + respostas do Google Forms"></textarea>
      <button type="submit">Importar inscritos</button>
    </form>
  </section>
  ...
  <section>
    ...
    <form method="POST" action="/admin/grade-sorteio-simples/gerar">
      ...
      <input name="adminSecret" type="password" placeholder="Digite a senha admin da Railway" required>
      <br><br>
      <label>Quantidade de horÃ¡rios vagos aleatÃ³rios por dia:</label>
      <input name="vacancyPerDay" type="number" min="0" max="24" value="0">
      <div class="muted">Use 0 se vocÃª quiser escolher os horÃ¡rios vagos manualmente.</div>
      <br><br>

      <label>HorÃ¡rios que vocÃª quer deixar VAGO:</label>
      <input name="manualVacantHours" placeholder="Exemplo: 12, 18, 23">
      <div class="muted">Digite sÃ³ as horas. Exemplo: 12, 18, 23. Se preencher aqui, o sistema deixa exatamente esses horÃ¡rios vagos.</div>
      <br><br>

      <label>HorÃ¡rios fixos por pessoa na semana:</label>
      <input name="maxWeekly" type="number" min="1" max="200" value="4">
      <br><br>

      <label>MÃ¡ximo por pessoa no dia:</label>
      <input name="maxDaily" type="number" min="1" max="24" value="2">
      <br><br>

      <label>Evitar repetir pessoa em horÃ¡rio seguido:</label>
  ...
  <section>
    <h2>4. Resumo rÃ¡pido da grade</h2>
      ...
      return `
        <table>
          ...
          <tbody>
            ...
            <tr>
              <td><span class="vago">VAGO</span> - reservado por vocÃª</td>
  ...
  <section>
    ...
    <div class="muted">
      SÃ³ clique aqui quando o resumo estiver correto. VAGO e PREENCHER MANUAL entram vazios na grade real.
    </div>
    ...
    <form method="POST" action="/admin/grade-sorteio-simples/aplicar">
      ...
      <select name="weekMode">
        <option value="next">PrÃ³xima semana</option>
        <option value="current">Semana atual</option>
        <option value="manual">Escolher data manual</option>
      </select>
      <br><br>

      <label>Data manual de inÃ­cio:</label>
      <input name="manualStartDate" type="date">
      <div class="muted">Use apenas se escolher "Escolher data manual". Se aplicar apenas um dia, essa serÃ¡ a data exata usada. Se aplicar semana toda, escolha uma segunda-feira.</div>
      <br><br>
      ...
      <select name="applyScope">
        <option value="semana">Semana toda</option>
        <option value="segunda">Apenas Segunda</option>
        <option value="terca">Apenas TerÃ§a</option>
        <option value="quarta">Apenas Quarta</option>
        <option value="quinta">Apenas Quinta</option>
        <option value="sexta">Apenas Sexta</option>
        <option value="sabado">Apenas SÃ¡bado</option>
      </select>
      <br><br>
  ...
  <section>
    ...
    <div class="muted">Rascunho. Ainda nÃ£o altera a grade ativa do app.</div>
    <table>
      <thead>
        <tr>
          <th>Dia</th>
  ...
  try {
    ...
    if (!pasted.trim()) {
      return res.type('html').send(fenixRenderGradeSorteioSimplesPage({
        message: 'Erro: o campo da planilha veio vazio. Cole cabeÃ§alho + respostas.'
      }));
    }
    ...
    if (!parsed.length) {
      return res.type('html').send(fenixRenderGradeSorteioSimplesPage({
        message: 'Erro: nenhum inscrito encontrado. Cole a planilha com cabeÃ§alho.'
      }));
    }
...
function fenixGenerateGradeDrawImprovedFinal(applicants, options = {}) {
  ...
  // Regra pensada para 50+ usuÃ¡rios:
  // Semana cheia tem 432 telas. Com 50 pessoas, mÃ©dia fica perto de 7/8 por pessoa.
...
app.post('/admin/grade-sorteio-melhorado/limpar-inscritos', (req, res, next) => {
  ...
  if (confirmText !== 'APAGAR') {
    return res.type('html').send(fenixRenderGradeSorteioSimplesPage({
      applicants: fenixReadFormApplicantsFileFinal(),
      draw: fenixReadGradeDrawFileFinal(),
      message: 'Para limpar todos os inscritos, digite APAGAR no campo de confirmaÃ§Ã£o.'
    }));
  }
...
app.post('/admin/grade-sorteio-simples/aplicar', fenixSimpleAdminAuth, (req, res) => {
  try {
    ...
    return res.type('html').send(fenixRenderGradeSorteioSimplesPage({
      applicants,
      draw,
      message: 'Grade aplicada no app. HorÃ¡rios salvos: ' + result.saved + ' | InÃ­cio: ' + result.weekStart + ' | Escopo: ' + result.applyScope
    }));
  } catch (error) {
    return res.type('html').send(fenixRenderGradeSorteioSimplesPage({
      applicants: fenixReadFormApplicantsFileFinal(),
