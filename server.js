...
app.post('/api/fenix/admin/user/delete', requireFenixAdmin, (req, res) => {
  ...
  if (confirm !== 'APAGAR') {
    return res.status(400).json({ ok: false, message: 'Digite APAGAR para confirmar.' });
  }

  if (username.toLowerCase() === 'gokuumods') {
    return res.status(403).json({ ok: false, message: 'Por seguranca, a conta admin GokuuMods nao pode ser excluida.' });
  }
  ...
  if (Array.isArray(data.sessions)) {
    data.sessions = data.sessions.filter((session) => {
      return session.userId !== userId && String(session.username || '').toLowerCase() !== username.toLowerCase();
    });
  }

  if (Array.isArray(data.fenixSessions)) {
    data.fenixSessions = data.fenixSessions.filter((session) => {
      return session.userId !== userId && String(session.username || '').toLowerCase() !== username.toLowerCase();
    });
  }

  if (Array.isArray(data.loginSessions)) {
    data.loginSessions = data.loginSessions.filter((session) => {
      return session.userId !== userId && String(session.username || '').toLowerCase() !== username.toLowerCase();
    });
  }
  ...
  res.json({
    ok: true,
    message: 'Conta excluida com sucesso. Se ela estava logada, tera que criar/entrar novamente.',
    deleted: {
      username: user.username,
      removedUsers: oldCount - data.users.length
    }
  });
});
