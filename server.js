...
function fenixNormalizeExtraTargetFinal(number, value) {
  ...
  const hasStartedAny = schedules.some((s) => s.start && nowISO >= s.start);
  const enabled = activeSched ? Boolean(name && url) : (hasStartedAny ? false : Boolean(extra.enabled && url));
  return {
    number, enabled, name,
    url: enabled ? url : '',
...
app.post('/api/fenix/admin/schedule/bulk', requireFenixAdmin, (req, res) => {
  ...
  function buildUrl(name) {
    ...
    return fenixRaffleWinnerUrl143(value.replace(/^@/, ''));
  }
...
// FENIX_GRADE_RAFFLE_DRAW_LOGIC_FINAL
function fenixRaffleWeightForUser143(username, wins) {
  const w = wins && typeof wins === 'object' ? Number(wins[String(username).toLowerCase()] || 0) : 0;
  const weight = 3 - w;
  return weight < 1 ? 1 : weight;
}

function fenixRaffleDrawOne143(candidates, wins) {
  const pool = [];
  for (const cand of (Array.isArray(candidates) ? candidates : [])) {
    const weight = fenixRaffleWeightForUser143(cand, wins);
    for (let i = 0; i < weight; i += 1) pool.push(cand);
  }
...
function fenixRaffleResolveSlot143(slot, participants, wins, guaranteed, recentWinners) {
  ...
  const alreadyInSlotNames = [];
  for (let screen = 1; screen <= 3; screen += 1) {
    const name = String(slot['screen' + screen + 'Name'] || '').toLowerCase();
    if (name) alreadyInSlotNames.push(name);
  }
  candidates = candidates.filter((c) => !alreadyInSlotNames.includes(String(c).toLowerCase()));

  // Para cada tela vazia, sorteia um ganhador (sem repetir)
  for (let screen = 1; screen <= 3; screen += 1) {
    const name = String(slot['screen' + screen + 'Name'] || '');
    ...
    candidates = candidates.filter((c) => String(c).toLowerCase() !== String(winner).toLowerCase());
  }
...
function fenixProcessRaffles143() {
  ...
  if (raffle.weekStart !== civilWeekStart) {
    // Semana virou: zera inscricoes e vitorias
    fenixSaveGradeRaffleFinal({ weekStart: civilWeekStart, slots: {}, wins: {} });
    return { processed: 0, filled: 0, note: 'semana-nova-reset' };
  }

  const slots = raffle.slots && typeof raffle.slots === 'object' ? raffle.slots : {};
  const wins = raffle.wins && typeof raffle.wins === 'object' ? raffle.wins : {};
  const resolved = raffle.resolved && typeof raffle.resolved === 'object' ? raffle.resolved : {};
  ...
  for (const slotKey of orderedSlotKeys) {
    ...
    if (date < civilWeekStart || date > civilWeekEnd) continue;

    const slotTime = fenixSlotDateTimeFinal(date, hour);
    if (slotTime <= 0) continue;

    // So processa depois que fechou (dentro dos 15 min finais ou depois)
    if (slotTime - now > FENIX_GRADE_RAFFLE_CUTOFF_MS_143) continue;

    const slot = data.schedule.find((s) => s.slotDate === date && s.slotHour === hour);
    ...
    if (winnersLog.length) {
      slot.updatedAt = new Date().toISOString();
      slot.source = 'grade-sorteio-auto';
    }

    resolved[slotKey] = { at: now, winners: winnersLog };
  }
...
app.get('/admin/sorteio-central/vagas', (req, res) => {
  const nowBr = new Date(Date.now() - 3 * 60 * 60 * 1000);
...
app.get('/api/fenix/admin/fix-urls', (req, res, next) => { req.headers['x-fenix-admin'] = 'GokuuMods'; req.headers['x-fenix-admin-secret'] = String(req.query?.adminSecret || '').trim(); next(); }, requireFenixAdmin, (req, res) => {
  ...
  for (const slot of data.schedule) {
    for (let screen = 1; screen <= 3; screen += 1) {
      const name = String(slot['screen' + screen + 'Name'] || '').trim();
      if (!name) continue;
      const oldUrl = String(slot['screen' + screen + 'Url'] || '');
      const newUrl = fenixRaffleWinnerUrl143(name);
      if (newUrl && newUrl !== oldUrl) {
        slot['screen' + screen + 'Url'] = newUrl;
        fixed += 1;
      }
    }
  }
  if (fixed > 0) writeFenixData(data);
...
app.get('/fenix/grade/raffle/run-now', (req, res, next) => { req.headers['x-fenix-admin'] = 'GokuuMods'; req.headers['x-fenix-admin-secret'] = String(req.query?.adminSecret || '').trim(); next(); }, requireFenixAdmin, (req, res) => {
  try {
    ...
    console.error('Erro no sorteio manual:', error);
...
// FENIX_SORTEIO_CENTRAL_PAGE
function fenixRenderSorteioCentralPage({ participants = [], guaranteed = {}, wins = {}, message = '', adminSecret = '' } = {}) {
  const rows = participants.map((name) => {
    ...
    return '<tr>'
      ...
      + '<input type="hidden" name="adminSecret" value="' + String(adminSecret || '').replace(/"/g, '&quot;') + '" class="js-secret" />'
...
app.post('/admin/sorteio-central/ver', fenixSimpleAdminAuth, (req, res) => {
  const raffle = fenixReadGradeRaffleFinal();
  const applicants = fenixReadFormApplicantsFileFinal();
  const participants = (Array.isArray(applicants) ? applicants : []).map((a) => String(a.nick || '').trim()).filter(Boolean).sort((a, b) => a.localeCompare(b, 'pt-BR'));
  const adminSecret = String(req.body?.adminSecret || '').trim();
  res.type('html').send(fenixRenderSorteioCentralPage({ participants, guaranteed: raffle.guaranteed || {}, wins: raffle.wins || {}, adminSecret }));
});
...
app.post('/admin/sorteio-central/toggle', fenixSimpleAdminAuth, (req, res) => {
  ...
  res.type('html').send(fenixRenderSorteioCentralPage({ participants, guaranteed: raffle.guaranteed, wins: raffle.wins || {}, message: msg, adminSecret: String(req.body?.adminSecret || '').trim() }));
});
