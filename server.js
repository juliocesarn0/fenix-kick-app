...
// FENIX_ADMIN_DELETE_USER_FINAL
app.post('/api/fenix/admin/delete-user', requireFenixAdmin, (req, res) => {
  ...
  const user = data.users.find((item) => {
    return String(item.username || '').toLowerCase() === target;
  });
  ...
  const kickUsername = String(user.kickUsername || user.kickName || user.username || '').toLowerCase();

  data.users = data.users.filter((item) => {
    return String(item.id) !== String(userId) &&
      String(item.username || '').toLowerCase() !== target;
  });
  ...
  data.cycles = data.cycles.filter((item) => {
    const cycleUser =
      item.username ||
      item.userName ||
      item.farmerUserName ||
    ...
    return String(item.userId) !== String(userId) &&
      String(cycleUser || '').toLowerCase() !== target;
  });

  data.farmHeartbeats = data.farmHeartbeats.filter((item) => {
    return String(item.userId) !== String(userId) &&
      String(item.username || '').toLowerCase() !== target;
  });

  data.deviceLocks = data.deviceLocks.filter((item) => {
    return String(item.userId) !== String(userId) &&
      String(item.username || '').toLowerCase() !== target;
  });

  data.kickLocks = data.kickLocks.filter((item) => {
    const lockUser = String(item.username || item.fenixUsername || '').toLowerCase();
    const lockKick = String(item.kickUsername || item.kickName || item.kickSlug || '').toLowerCase();

    return lockUser !== target && lockKick !== kickUsername;
  });
  ...
  return res.json({
    ok: true,
    message: 'Usuario apagado completamente.',
